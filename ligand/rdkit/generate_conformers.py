...
if 'ANTECHAMBER_BIN' in os.environ:
    antechamber_bin = os.environ['ANTECHAMBER_BIN']
else: 
    antechamber_bin = os.popen('which antechamber').read().strip()
...
def dump_conformers_sdf(mol, output, conf_ids=None, 
    ...
    if True:
        for idx, conf in enumerate(conformers):
            ...
            block = MolToPDBBlock(mol, confId=conf_id)

            with open(f"{conf_name}.pdb",'w') as ofp:
                ofp.write(block)
            os.system(f"{antechamber_bin} -i {conf_name}.pdb -fi pdb -o {conf_name}.mol2 -fo mol2 -at sybyl")
            os.system("rm A*")
...
def main(args, log=logging):
    ...
    with open(params.o,'w') as output:
        names = dump_conformers_sdf(embedded, output, conf_ids=sorted_by_energy, 
                                                      renumber=True, template = template)
